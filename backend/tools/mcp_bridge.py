...
# 分级超时常量（D19 / 设计 07 §7.2）：MCP 调用默认 15s
try:
    from timeouts import MCP_TIMEOUT
except ImportError:  # cwd=repo root
    from backend.timeouts import MCP_TIMEOUT
...
class MCPToolBridge:
    ...
    def __init__(self, mcp_config_path: str = "") -> None:
        ...
        self._timeout: float = MCP_TIMEOUT
        ...
        if config_path.exists():
            try:
                ...
                self._timeout = self._config.get("timeout_seconds", MCP_TIMEOUT)
    ...
    # MCP 网络错误重试策略（07-tool-registry.md §7.1：重试 3 次指数退避）
    _MAX_RETRIES: int = 3
    _RETRY_BASE_DELAY: float = 0.5  # 秒；第 n 次重试等待 base * 2**(n-1)

    async def call_tool(self, server_name: str, tool_name: str, args: dict) -> dict:
        """
        调用指定 MCP 服务器上的工具（HTTP POST），带指数退避重试。
        网络/5xx 错误重试至多 _MAX_RETRIES 次；4xx 等确定性错误不重试。
        ...
        for attempt in range(1, self._MAX_RETRIES + 1):
            try:
                async with aiohttp.ClientSession(
                    ...
                    async with session.post(url, json=payload) as resp:
                        ...
                        # 4xx 客户端错误不重试（确定性失败）；5xx 服务端错误重试
            ...
            if attempt < self._MAX_RETRIES:
                ...
                logger.debug(
                    "MCPToolBridge: %s/%s 第 %d 次失败（%s），%.1fs 后重试",
                    server_name, tool_name, attempt, last_error, delay,
                )
                await asyncio.sleep(delay)

        logger.warning(
            f"MCPToolBridge: 调用 {server_name}/{tool_name} 失败（重试 {self._MAX_RETRIES} 次）— {last_error}"
        )
        return {"error": last_error, "mcp_server": server_name}

    def unregister_from_registry(self, registry: "ToolRegistry") -> int:
        """
        从 ToolRegistry 注销本桥接器注册的所有 MCP 工具（热卸载 / 服务断开时调用）。
        返回注销的工具数量。
        """
        count = 0
        for server in self._servers:
            ...
            for tool_name in server.get("tools", []):
                ...
                if registry.unregister(full_name):
                    count += 1
            for tool in self._dynamic_tools.get(server_name, []):
                full_name = f"mcp_{server_name}_{tool.get('name', '')}"
                if registry.unregister(full_name):
                    count += 1
        return count
        ...
        for server in self._servers:
            ...
            for tool_name in server.get("tools", []):
                ...
                tool_def = ToolDef(
                    ...
                    # 注：静态 mcp.json 仅提供工具名，无 per-tool inputSchema，
                    # 故用通用 args 对象占位；真实 JSON Schema 仅在动态发现路径
                    # （register_plugin_mcp_servers，取 tool["inputSchema"]）下可用。
                    parameters={
                        "type": "object",
                        "properties": {
                            "args": {
